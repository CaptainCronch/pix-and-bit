...
func init():
	var ui = Global.bit_camera.get_node("UI")
	_frisbee_anim = ui.get_node("Crosshair/FrisbeeAnim")
	_crosshair_anim = ui.get_node("Crosshair/CrosshairAnim")
	_right_arrow = ui.get_node("Crosshair/Right")
	_left_arrow = ui.get_node("Crosshair/Left")
	_down_arrow = ui.get_node("Crosshair/Down")
	_up_arrow = ui.get_node("Crosshair/Up")
...
func get_input():
	analog_rotation = Vector2(
			Input.get_axis("bit_look_left", "bit_look_right"),
			Input.get_axis("bit_look_down", "bit_look_up"))
	
	_move_dir = Vector3.ZERO
	
	_move_dir.x = Input.get_axis("bit_left", "bit_right")
	_move_dir.z = Input.get_axis("bit_forward", "bit_back")
	_move_dir = _move_dir.rotated(Vector3.UP, _spring_arm.rotation.y).normalized()
	
	if Input.is_action_just_pressed("bit_secondary"):
		aiming = true
		_spring_arm.aim_zoom = _spring_arm.max_aim_zoom
	elif Input.is_action_just_released("bit_secondary"):
		aiming = false
		_spring_arm.aim_zoom = 0
	
	if Input.is_action_just_pressed("bit_primary") and aiming:
		hold_frisbee()
	elif Input.is_action_just_released("bit_primary") and aiming:
		shoot_frisbee()
	if Input.is_action_pressed("bit_primary") and aiming:
		shoot_direction()
	
	if Input.is_action_just_pressed("bit_menu"):
...
func jumping(delta):
	...
	if Input.is_action_just_pressed("bit_jump"):
	...
	if not Input.is_action_pressed("bit_jump") and _velocity.y > jump_force * 0.5:
